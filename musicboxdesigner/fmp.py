...
class FmpTrack:
    ...
    def set_velocity(self, velocity: float) -> None:
        if not 0 <= velocity <= 1:
            raise ValueError('velocity must be a float between 0 and 1.')
        self.notes = [note.__class__(note.pitch, note.time, note.duration, velocity)
                      for note in self.notes]
        ...
        for _ in range(track_count):
            ...
            for _ in range(note_count):
                ...
                if duration_tick == 0:
                    ...
                    duration = duration_tick / fmp_file.ticks_per_beat
            ...
            for message in midi_track:
                ...
                if message.type == 'note_on':
                    if message.velocity > 0:
                        ...
                        fmp_track.notes.append(FmpNote(message.note, time, DEFAULT_DURATION, message.velocity / 127))
            if fmp_track:
            ...
            if isinstance(time_mark, FmpBpmTimeSignatureMark):
                if time_mark.change_tempo:
                    tempo_track.append(MetaMessage(type='set_tempo',
                                                   tempo=bpm2tempo(time_mark.bpm),
                                                   time=time_mark.time * ticks_per_beat))
