...
def export_pic(file,
    ...
    参数 filename: 输出图片的文件名，留空则取参数file的文件名
    参数 papersize: 字符串或字典
                可以使用PAPER_INFO中的预设值(例如'A4_vertical')
                也可以使用字典来自定义，格式为
                {'size': 一个元组(宽,高)，单位毫米,
                 'col': 一页的分栏数,
                 'row': 一栏的行数}
                    ...
                    if msg.velocity > 0:
                        if msg.note + transposition in PITCH_TO_MBNUM:
                            notes.append([PITCH_TO_MBNUM[msg.note + transposition],
                                          miditime / ticks_per_beat * 8 * scale])
        notes.sort(key=lambda a: (a[1], a[0]))
        length = notes[-1][1]
        return notes, length
    ...
    elif typ == emid.EmidFile:
        if filename is None:
            filename = os.path.splitext(file.filename)[0]
        notes, length = process_emidfile(file)

    elif typ == mido.MidiFile:
        if filename is None:
            filename = os.path.splitext(file.filename)[0]
    ...
    pages = math.floor(length / (col * row * 8)) + 1
    ...
    if font is None:
        for i in FONT_PATH:
            try:
                font0 = ImageFont.truetype(i, round(mm2pixel(3.4)))
                font1 = ImageFont.truetype(i, round(mm2pixel(6)))
                break
            except:
                pass
    else:
        font0 = ImageFont.truetype(font, round(mm2pixel(3.4)))
        font1 = ImageFont.truetype(font, round(mm2pixel(6)))

    for i in range(pages):
        image = Image.new('RGBA', posconvert(size), (0, 0, 0, 0))
        ...
        for j in range(col if i < pages - 1 else cols):
            ...
            draw.text(xy=posconvert((startpos[0] + 70*j + 6, endpos[1])),
                      ...
                      fill=(0, 0, 0, 255))
            '曲目标题'
            for k, char in enumerate(filename):
    ...
    if type(background) == str:
        bgimage = Image.open(background).resize(
            (posconvert(size)), Image.BICUBIC).convert('RGBA')
    elif type(background) == Image.Image:
        bgimage = background.resize(
            (posconvert(size)), Image.BICUBIC).convert('RGBA')
    elif type(background) == tuple:
        bgimage = Image.new('RGBA', posconvert(size), background)

    result = []
    for pagenum, image in enumerate(images):
        cpimage = Image.alpha_composite(bgimage, image)
        if save_pic:
            save_path = filename + '_' + str(pagenum + 1) + '.png'
            if not overwrite:
...
def batch_export_pics(path=None,
                      ...
                      font=None):
    '''
    批量将path目录下的所有.mid和.emid文件转换为图片
    如果path参数留空，则取当前工作目录
    '''
    if path is None:
        path = os.getcwd()
...
if __name__ == '__main__':
    batch_export_pics(overwrite=True)
