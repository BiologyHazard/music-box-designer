...
EMID_PITCHES: list[int] = [93, 91, 89, 88, 87, 86, 85, 84, 83, 82,
                           ...
                           71, 70, 69, 67, 65, 64, 62, 60, 55, 53]  # fmt: off
...
class EmidFile:
    ...
    def get_length(self) -> int:
        return (
            math.ceil(
                max(note.tick for track in self.tracks for note in track.notes)
                / EMID_TICKS_PER_BEAT
                * 2
            )
            + 1
            ...
            for message in midi_track:
                ...
                if message.type == "note_on" and message.velocity > 0:
                    if message.note + transposition not in EMID_PITCHES:
                        logger.warning(
                            f"note {message.note + transposition} out of range!"
                        )
                        continue
                    tick: int = round(
                        midi_tick / midi_file.ticks_per_beat * EMID_TICKS_PER_BEAT
                    )
                    emid_track.notes.append(
                        EmidNote(EMID_PITCHES.index(message.note + transposition), tick)
                    )
            if emid_track.notes:
    ...
    def export_midi(
        ...
        *,
        ...
        if bpm is not None:
            ...
            tempo_track.append(
                MetaMessage(type="set_tempo", tempo=bpm2tempo(bpm), time=0)
            )
            ...
            for note in sorted(track.notes, key=lambda note: note.tick):
                ...
                midi_track.append(
                    Message(
                        ...
                        note=EMID_PITCHES[note.emid_pitch] + transposition,
                        time=round(
                            (note.tick / EMID_TICKS_PER_BEAT + DEFAULT_DURATION)
                            * ticks_per_beat
                        ),
                    )
                )
